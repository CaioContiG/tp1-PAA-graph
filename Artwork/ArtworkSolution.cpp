...
using namespace std;
 ...
 // Only the Node class would be sufficient, this is not optimal
class Sensor{
    public:
        int x;
        int y;
...
int main() {
    ...
    // Scan graph from wall 0
    graph[0].visited = -1;
    DFS(graph,0);
    int result = 1;

    // If wall 0 finds 2 or 1, is blocked
    if(graph[1].parent != -2 || graph[2].parent != -2){result = 0;} 
        ...
        // Scan graph from wall 3
        DFS(graph,3);
        // If wall 3 finds 2 or 1, is blocked
        if(graph[1].parent != -2 || graph[2].parent != -2){result = 0;}
    }
